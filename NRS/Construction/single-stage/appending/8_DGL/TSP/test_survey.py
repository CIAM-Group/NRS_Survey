...
def main():
    ...
    # ! 补充文件读取地址变量
    bench_path = "/public/home/bayp/exp_survey_202509/0_data_survey/survey_bench_tsp"
    ...
    tsplib_data = [(name, TSPLIBReader(os.path.join(bench_path, name + '.tsp'))[1]) for name in tsplib_names]
    # 按照size排序
    ...
    logger.info(f"TSP [0, 1000): {len(st1)} instances, avg_gap {avg_list(st1)*100:.3f}%, avg_time {sum(time_st1)/len(time_st1):.2f}s")
    logger.info(f"TSP [1000, 10000): {len(st2)} instances, avg_gap {avg_list(st2)*100:.3f}%, avg_time {sum(time_st2)/len(time_st2):.2f}s")
    logger.info(f"TSP [10000, 100000]: {len(st3)} instances, avg_gap {avg_list(st3)*100:.3f}%, avg_time {sum(time_st3)/len(time_st3):.2f}s")
