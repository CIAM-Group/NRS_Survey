...
bench_path = '/public/home/bayp/exp_survey_202509/0_data_survey/survey_bench_cvrp'
...
def main():
    ...
    cvrplib_names, instance_sizes = zip(*sorted(zip(cvrplib_names, instance_sizes), key=lambda x: x[1]))
    ...
    for i in range(len(cvrplib_names)):
        ...
        # 每个范围结束后的统计
        next_size = instance_sizes[i+1]
        if ((size < 1000 and 1000 <= next_size < 10000) or
    ...
    logger.info(f"CVRP [0, 1000): {len(st1)} instances, avg_gap {avg_list(st1)*100:.3f}%, avg_time {sum(time_st1)/len(time_st1):.2f}s")
    logger.info(f"CVRP [1000, 10000): {len(st2)} instances, avg_gap {avg_list(st2)*100:.3f}%, avg_time {sum(time_st2)/len(time_st2):.2f}s")
    logger.info(f"CVRP [10000, 100000]: {len(st3)} instances, avg_gap {avg_list(st3)*100:.3f}%, avg_time {sum(time_st3)/len(time_st3):.2f}s")
